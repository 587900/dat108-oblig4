...
public abstract class DAO<T extends IHasPrimaryKey> {
	...
	//Note: due to how transactions are handled with JTA, it is impossible for .getReference to get null, and then for .persist to claim an entity already exists. 
	/** persists an entity in the database, returns true on success, false if entity already exists */
	public boolean create(T entity) {
		if (exists(entity.getPrimaryKey())) return false;
		em.persist(entity);
		return true;
	}
	...
	public boolean exists(Object id) {
		return em.getReference(clazz, id) != null;
	}
